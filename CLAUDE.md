...
- Repo status: **Phase 0 complete.** Phase 1 (corpus construction) — **Milestones 1–3 complete and reviewed** (each: task reviews + whole-milestone review + independent adversarial review; all on `main`, not pushed):
  ...
  - **M4 Contextual Retrieval — CODE COMPLETE + fully reviewed; the generation run has NOT been done.** `ingest/context_windows.py` (pure, deterministic token-budgeted windows) + `ingest/contextualize.py` (pin store, Haiku 4.5 generator, `contexts_for_document`, cost estimator, `context_coverage` gate, CLI) + `build.py` wiring. **131 tests passing**, per-task reviews + whole-branch review (opus) all clean. `corpus/contexts/` is still empty, so `chunks.context` is still NULL: generating it needs an `ANTHROPIC_API_KEY` and costs **~$8.26 one-time** (measured with `python -m kbmcp.ingest.contextualize --estimate`, verified independently; user approved the spend, key not yet available). Run `python -m kbmcp.ingest.contextualize` then `python -m kbmcp.ingest.build --force --require-context`, commit `corpus/contexts/*.json`.
  ...
  - **Next: M5 cross-ref graph** (`citations.py` + `graph.py` — deterministic reference extraction → resolve to in-corpus target chunk → cross-document `references` edges; the T4 multi-hop enabler). Remaining after: BM25 index + `build_corpus.py` (M6), benchmark + gold resolution (M7).
- Working conventions:
